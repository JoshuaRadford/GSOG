...
public class TilemapVisual : MonoBehaviour {
    ...
    public void SetGrid(TilemapSystem tilemap, GridSystem<Tile> grid) {
        this.grid = grid;
        UpdateMapVisual();
    ...
    private void Grid_OnGridValueChanged(object sender, GridSystem<Tile>.OnGridObjectChangedEventArgs e) {
        updateMesh = true;
    }
    ...
    private void UpdateMapVisual() {
        ...
        for (int x = 0; x < grid.GetWidth(); x++) {
            for (int y = 0; y < grid.GetHeight(); y++) {
                ...
                TilemapSprite tilemapSprite = tile.GetTilemapSprite();
